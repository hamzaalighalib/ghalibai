...
evaluateMathExpression(userMessage);
        response = `Your result: ${calculationResult} - <br>`;
    }

    if 

(userMessage.includes("oh") || userMessage.includes("oops")) {
...

{
        ...
        saveInformationToJSON(userMessage, 

informationSentences);
        response += ` I think you may know about ${informationSentences.join(" ")}. Please provide me more 

information about this, so I can remember it for next time.`;
    }
...
$("div.BNeawe.vvjwJb.AP7Wnd").first().text();
...
= extractVerbs(userMessage);

    if (nouns.length > 0 && verbs.length > 0) {
        return `It appears you're inquiring about 

${nouns.join(" ")} and expressing interest in ${verbs.join(" ")}.`;
    } else if (nouns.length > 0) {
        return `You've 

mentioned ${nouns.join(" ")}. Can you provide more details on that?`;
    } else if (verbs.length > 0) {
        return `You seem 

interested in ${verbs.join(" ")}. What specific information are you seeking?`;
    } else {
        return "I'm sorry, I didn't quite 

catch that. Could you please provide more context?";
    }
}

function extractNouns(userMessage) {
    const taggedWords = new 

pos.Lexer().lex(userMessage);
...
function extractVerbs(userMessage) {
    const taggedWords = new pos.Lexer().lex
...
(userMessage) {
    ...
    return sentences.filter((sentence) => sentence.toLowerCase
...
saveInformationToJSON(uq, informationSentences) {
    const combinedHTML = informationSentences.join(" ");
    const escapedHTML = 

escapeHTML(combinedHTML);
...
generateRandomGreetingResponse() {
    const greetings = [
        "Hello!",
        "Hi there!",
        "Hey!",
...
lowerCaseMessage = userMessage.toLowerCase(); // Convert user message to lowercase once
